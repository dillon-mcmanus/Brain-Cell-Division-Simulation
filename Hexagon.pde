public class Hexagon{
   ...
   public Hexagon(Hexagon_List HL){
     VL=HL.VL;
     isInhibited=false;
     open_vert=0;
     c=color(255);
   ...
   Vertex getVertex(int i){  // i is from 1 to 6
   ...
   //^^^^^AsymmetricSplitCell
      ...
      void preview(int rand_vert, int which_inhibit){   //rand_vert is 0, 1, 2;  which_inhibit is 0, 1
  ...
  void moveVertices0(int rand_vert, int rand_side){
    ...
    if(rand_side==0){
      ...
      this.moveVertices(v0, v3, kpinch);   //kcontract=.9
      //save verts 1 and 2. 
      Vertex va=this.getVertex(v1);
      Vertex vb=this.getVertex(v2);
      //move 1 and 2 closer to 4 and 5 respectively. 
      this.moveVerticesAway(v1, v4, kcontract);    //kcontract = .75
      this.moveVerticesAway(v2, v5, kcontract);
      // move 4 and 5 farther from 1 and 2 respectively.
      this.moveVerticesAway(v4, va, kexpand);    //kexpand=1.2
      this.moveVerticesAway(v5, vb, kexpand);
    }
    else if(rand_side==1){
      ...
      this.moveVerticesAway(v4, v1, kcontract);
      this.moveVerticesAway(v5, v2, kcontract);
      // move 1 and 2 farther from 4 and 5 respectively.
      this.moveVerticesAway(v1, va, kexpand);
      this.moveVerticesAway(v2, vb, kexpand);
    }
    else{
